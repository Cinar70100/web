...
// Demo veri
const offices = [
  { id: 1, name: "Aydın Web Yazılım", username: "aydinweb", category: "Emlak", isSuperOffice: true, expiryDate: "25 Ekim 2026", status: "Aktif" },
  { id: 2, name: "Karaman Emlak", username: "karamanemlak", category: "Emlak", isSuperOffice: false, expiryDate: "15 Aralık 2025", status: "Pasif"},
  { id: 3, name: "Güven Emlak", username: "guvenemlak", category: "Emlak", isSuperOffice: false, expiryDate: "01 Ocak 2026", status: "Onay Bekliyor" },
];

export default function AllOfficesPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            ...
            <div>
              ...
              <select id="sort" name="sort" className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                 <option value="">Seçiniz</option> <option value="name_asc">Ad (A-Z)</option> <option value="name_desc">Ad (Z-A)</option> <option value="date_desc">Bitiş Tarihi (Yeni)</option> <option value="date_asc">Bitiş Tarihi (Eski)</option>
              </select>
            </div>
            <div>
      ...
      {/* Tablo Kartı */}
      <DashboardCard title="Tüm Emlak Ofisleri">
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {offices.map((office) => (
                <tr key={office.id} className="hover:bg-gray-50">
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{office.expiryDate}</td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm">
                    <Badge text={office.status} type={office.status === 'Aktif' ? 'success' : (office.status === 'Onay Bekliyor' ? 'warning' : 'danger')} />
                  </td>
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm font-medium">
                    <div className="flex space-x-3">
                      ...
                      >
                       ...
                       {/* ... (Diğer ikonlar aynı) ... */}
                       {office.status === 'Aktif' ? ( <button className="text-yellow-600 hover:text-yellow-900" title="Pasifleştir"><Power size={18} /></button>
                      ) : office.status === 'Onay Bekliyor' ? ( <button className="text-teal-600 hover:text-teal-900" title="Onayla"><CheckCircle size={18} /> </button>
                       ) : ( <button className="text-yellow-600 hover:text-yellow-900" title="Aktifleştir"><CheckCircle size={18} /> </button> )}
